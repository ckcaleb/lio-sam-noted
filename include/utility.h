...
class ParamServer
{
    ...
    float imuRPYWeight;  //imu 和lidar 位姿融合过程imu 位姿权重
    vector<double> extRotV;
    vector<double> extRPYV;
    vector<double> extTransV;
    //Rli 从雷达坐标系到imu 坐标系的旋转矩阵
    Eigen::Matrix3d extRot; 
    //Rli 从雷达坐标系到imu 坐标系的旋转矩阵
    Eigen::Matrix3d extRPY;     
   // Tli 从雷达坐标系到imu 坐标系的坐标系平移
    Eigen::Vector3d extTrans;  
    //Rli 从雷达坐标系到imu坐标系的旋转四元数
    ...
    float z_tollerance; //z轴约束
    float rotation_tollerance; //roll pitch值约束
    ...
    float surroundingKeyframeDensity; //下采样半径
    float surroundingKeyframeSearchRadius;//kdtree 查询半径
    
    // Loop closure
    bool  loopClosureEnableFlag; //是否需要回环检测 
    float loopClosureFrequency;//回环检测频率
    int   surroundingKeyframeSize;
    float historyKeyframeSearchRadius;//回环kdtree 半径
    float historyKeyframeSearchTimeDiff;
    int   historyKeyframeSearchNum; //回环中提取周围关键帧数
    float historyKeyframeFitnessScore;//判断icp 得分是否满足要求

    // global map visualization radius
    float globalMapVisualizationSearchRadius; //回环kdtree 半径
    ...
    // 从param server中读取key为"lio_sam/pointCloudTopic"对应的参数，存pointCloudTopic，第三个参数是默认值
        ...
        //only 旋转矩阵，相当于转换到同一个坐标系方向，不同零点

        extRot = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(extRotV.data(), 3, 3);//https://zhuanlan.zhihu.com/p/517228684 行列转换 
        ...
        nh.param<int>("lio_sam/numberOfCores", numberOfCores, 4);
    ...
    /**
     * @description: 原始测量数据转换到lidar坐标系 
     * @param {Imu&} imu_in imu原始测量数据 
     * @return {Imu}imu_out 返回lidar 坐标系下IMU数据
     */
    sensor_msgs::Imu imuConverter(const sensor_msgs::Imu& imu_in)
    {
        ...
        // 雷达坐标系到世界坐标系四元数
        Eigen::Quaterniond q_final = q_from *extQRPY.inverse();//https://github.com/TixiaoShan/LIO-SAM/issues/335
